...
from utils.utils_2p5Dv2 import ImageToImage2p5DRM_Dynamic_INSTANCE, ImageToImage2p5DRM_Dynamic_Prostate
...
def main():
    ...
    tf_val = JointTransform2p5D(img_size=opt.img_size, crop=opt.crop, p_flip=0, p_gama=0, color_jitter_params=None,
                              long_mask=True, half_resize=False)  # image reprocessing
    train_dataset = ImageToImage2p5DRM_Dynamic_INSTANCE(opt.data_path, opt.train_split, tf_train, opt.classes, opt.assist_slice_number, opt.img_size, opt.assis_slice_inter)
    val_dataset = ImageToImage2p5DRM_Dynamic_INSTANCE(opt.data_path, opt.val_split, tf_val, opt.classes, opt.assist_slice_number, opt.img_size, opt.assis_slice_inter)  # return image, mask, and filename
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (input_image, ground_truth, mask_mini_slices, image_slices, gt_bbox, gt_label, *rest) in enumerate(trainloader):
           ...
           # output, mclass, class_loc, class_score = model(image_slices) # (b 1 256 256) and (b asn 1 256 256)
            output, mclassd5, mclassd4 = model(image_slices)
