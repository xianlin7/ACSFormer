...
class DC_and_CE_loss(nn.Module):
    ...
    def forward(self, net_output, target, all=False):
        """
...
class GODiceLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.):
        """
        """
        super(GODiceLoss, self).__init__()
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        tp_loss = -torch.log((tp + self.smooth)/(tp + fn + self.smooth))
        number_loss = torch.log(torch.abs(fp - fn) / (tp + fn + self.grad_factor) + 1)
        ...
        number_loss = number_loss.mean()

        return tp_loss + 0.2*number_loss, tp_loss, number_loss


class GODiceLoss2(nn.Module): # grad optimize
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.):
        """
        """
        super(GODiceLoss2, self).__init__()
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        tpg_loss = -torch.log((tp + self.smooth)/(tp + fn + self.smooth))
        tpp_loss = -torch.log((tp + self.smooth)/(tp + fp + self.smooth))
        ...
        tpg_loss = tpg_loss.mean()
        tpp_loss = tpp_loss.mean()

        return tpg_loss + 0.1*tpp_loss, tpg_loss, tpp_loss


class GODC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", weight_ce=1, weight_dice=1, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        :param ce_kwargs:
        ...
        :param weight_dice:
        """
        super(GODC_and_CE_loss, self).__init__()
        self.weight_dice = weight_dice
        self.weight_ce = weight_ce
        self.aggregate = aggregate
        self.ce = RobustCrossEntropyLoss(**ce_kwargs)
        self.ignore_label = ignore_label
        self.dc = GODiceLoss2(apply_nonlin=softmax_helper, **soft_dice_kwargs)
    ...
    def forward(self, net_output, target, all=False):
        ...
        dc_loss, tp_loss, number_loss = self.dc(net_output, target, loss_mask=mask) if self.weight_dice != 0 else 0
        ...
        if all:
            return result, ce_loss, dc_loss, tp_loss, number_loss
        else:
            return result
