...
import json
from utils.bbox_tools import obtain_gt_loc
...
class JointTransform2D:
    ...
    def __init__(self, img_size=256, crop=(32, 32), p_flip=0.0, p_rota=0.0, p_scale=0.0, p_gaussn=0.0, p_contr=0.0,
                 p_gama=0.0, p_distor=0.0, z_score=False, color_jitter_params=(0.1, 0.1, 0.1, 0.1), p_random_affine=0, half_resize=False,
                 long_mask=False):
        self.crop = crop
        self.p_flip = p_flip
        self.p_rota = p_rota
    ...
    def __call__(self, image, imagem2, mask):
        #  gamma enhancement
        if np.random.rand() < self.p_gama:
            ...
            imagem2 = (np.power(imagem2 / 255, 1.0 / g) / c) * 255
            imagem2 = imagem2.astype(np.uint8)
        # transforming to PIL image
        image, imagem2, mask = F.to_pil_image(image), F.to_pil_image(imagem2), F.to_pil_image(mask)
        # random crop
        if self.crop:
            i, j, h, w = T.RandomCrop.get_params(image, self.crop)
            image, imagem2, mask = F.crop(image, i, j, h, w), F.crop(imagem2, i, j, h, w), F.crop(mask, i, j, h, w)
        # random horizontal flip
        if np.random.rand() < self.p_flip:
            image, imagem2, mask = F.hflip(image), F.hflip(imagem2), F.hflip(mask)
        # random rotation
        if np.random.rand() < self.p_rota:
            angle = T.RandomRotation.get_params((-30, 30))
            image, imagem2, mask = F.rotate(image, angle), F.rotate(imagem2, angle), F.rotate(mask, angle)
        ...
        if np.random.rand() < self.p_scale:
            ...
            image, imagem2, mask = F.resize(image, (new_h, new_w), 2), F.resize(imagem2, (new_h, new_w), 2), F.resize(mask, (new_h, new_w), 0)
        ...
        if np.random.rand() < self.p_gaussn:
            ...
            imagem2 = np.array(imagem2) + noise
            imagem2[imagem2 > 255] = 255
            imagem2[imagem2 < 0] = 0
            imagem2 = F.to_pil_image(imagem2.astype('uint8'))
        # random change the contrast
        if np.random.rand() < self.p_contr:
            contr_tf = T.ColorJitter(contrast=(0.8, 2.0))
            image = contr_tf(image)
            imagem2 = contr_tf(imagem2)
        # random distortion
        if np.random.rand() < self.p_distortion:
            distortion = T.RandomAffine(0, None, None, (5, 30))
            image = distortion(image)
            imagem2 = distortion(imagem2)
        # color transforms || ONLY ON IMAGE
        if self.color_jitter_params:
            image = self.color_tf(image)
            imagem2 = self.color_tf(imagem2)
        # random affine transform
        if np.random.rand() < self.p_random_affine:
            affine_params = T.RandomAffine(180).get_params((-90, 90), (1, 1), (2, 2), (-45, 45), self.crop)
            image, imagem2, mask = F.affine(image, *affine_params), F.affine(imagem2, *affine_params), F.affine(mask, *affine_params)
        ...
        if self.half_resize:
            ...
            imagem2 = F.resize(imagem2, (self.img_size//2, self.img_size//2), 2)
        # transforming to tensor
        if self.zscore:
            image = norm_zscore(image)
            image = torch.from_numpy(image[None, :, :])
            imagem2 = norm_zscore(imagem2)
            imagem2 = torch.from_numpy(imagem2[None, :, :])
        else:
            image = F.to_tensor(image)
            imagem2 = F.to_tensor(imagem2)

        if not self.long_mask:
            mask = F.to_tensor(mask)
        else:
            mask = to_long_tensor(mask)
        return image, imagem2, mask
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image, imagem2, mask = correct_dims(image, imagem2, mask)
        if self.classes == 2:
            mask[mask > 1] = 1
        if self.joint_transform:
            image, imagem2, mask = self.joint_transform(image, imagem2, mask)
...
class ImageToImage2DR(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc(bbox, label, self.classes) # the original size
        # ------------------------------------------
        if self.one_hot_mask:
            assert self.one_hot_mask > 0, 'one_hot_mask must be nonnegative'
            mask = torch.zeros((self.one_hot_mask, mask.shape[1], mask.shape[2])).scatter_(0, mask.long(), 1)
        return image, mask, gt_class_loc, gt_class_label, id_ + '.png'
