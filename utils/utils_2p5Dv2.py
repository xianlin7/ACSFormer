...
import json
from utils.bbox_tools import obtain_gt_loc, obtain_gt_loc_MosMed, obtain_gt_loc_acdc, obtain_gt_loc_lits, obtain_gt_loc_prostate, obtain_gt_loc_ich, obtain_gt_loc_chaos, obtain_gt_loc_task09spleen
...
class JointTransform2p5D:
    ...
    def transform_image_mask(self, image, mask, random_dict):
        #  gamma enhancement
        r_gama = random_dict["r_gama"]
        if r_gama < self.p_gama:
            c = 1
...
class ImageToImage2p5D(Dataset):
    ...
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=2, assist_slice=3, img_size=256,
        ...
        with open(os.path.join(dataset_path, "start_slice.json"), 'r') as load_f:
            self.start_slice = json.load(load_f)
    ...
    def __getitem__(self, i):
        ...
        # if the id_ is background, then random sample from the following 5 slices
        if self.split[:5] == "train":
            if int(id_[3:]) < int(self.start_slice[id_[0:3]]):
                id_off = np.random.randint(0, 4)
                id_ = id_[0:3] + str(int(id_[3:]) + id_off).zfill(3)
...
class ImageToImage2p5DR(Dataset):
    """
    Reads the images and applies the augmentation transform on them.
    Usage:
        1. If used without the unet.model.Model wrapper, an instance of this object should be passed to
    ...
    def __getitem__(self, i):
        ...
        under_slices = torch.stack(under_slice, dim=0)
        assist_slices = torch.cat([under_slices, over_slices], dim=0)
        # ------------------------ find the object bbox ----------------------------
        bbox = list()
        label = list()
        for j in range(1, self.classes):
            index = torch.where(mask == j)
            if len(index[0]) > 0:
                label.append(j)
                miny, minx, maxy, maxx = torch.min(index[0]), torch.min(index[1]), torch.max(index[0]), torch.max(index[1])
                bbox.append([miny, minx, maxy, maxx])
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini, assist_slices, gt_class_loc, gt_class_label, id_ + '.png'

class ImageToImage2p5DRM(Dataset):
    """
    Reads the images and applies the augmentation transform on them.
    Usage:
        1. If used without the unet.model.Model wrapper, an instance of this object should be passed to
    ...
    def __getitem__(self, i):
        ...
        # ---------------------------- preprocess the assist slice ------------------------
        image_slices, mask_mini_slices = [], []
        image_slices.append(image)
        mask_mini_slices.append(mask_mini)
        patient_id, slice_id = id_[:3], int(id_[3:6])
        for i in range(self.assist_slice_number, 0, -1):
            underi = max(slice_id - i*self.inter, 0)
            underi_id = patient_id + str(int(underi)).zfill(3)
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
            if self.classes == 2:
                maski[maski > 1] = 1
            if self.joint_transform:
                imagei, mask_minii = self.joint_transform.transform_image_mask(imagei, maski, random_dict)
            image_slices.append(imagei)
            mask_mini_slices.append(mask_minii)
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            overi_id = patient_id + str(int(overi)).zfill(3)
            #print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
            if self.classes == 2:
                maski[maski > 1] = 1
            if self.joint_transform:
            ...
            image_slices.append(imagei)
            mask_mini_slices.append(mask_minii)
        image_slices = torch.stack(image_slices, dim=0) # k 1 h w
        mask_mini_slices = torch.stack(mask_mini_slices, dim=0)
        # ------------------------ find the object bbox ----------------------------
        bbox = list()
        label = list()
        for j in range(1, self.classes):
        ...
        gt_class_loc, gt_class_label = obtain_gt_loc(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'


class ImageToImage2p5DRM_Dynamic(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
        1. If used without the unet.model.Model wrapper, an instance of this object should be passed to
           torch.utils.data.DataLoader. Iterating through this returns the tuple of image, mask and image
...
class ImageToImage2p5DRM_Dynamic_ACDC(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=2, assist_slice=3, img_size=256,
        ...
        patient_slice_path = os.path.join(dataset_path, "patient_slice.json")
        self.patient_slice_number = read_slice_number(patient_slice_path)
        self.assist_slice_number = 1 # 3for {1, 3}, 6 for {1,3,5} , 9 for {1, 3, 5, 7}
        self.slice_class_number = ncslice
        self.inter = 1
        self.classes = classes
        self.one_hot_mask = one_hot_mask
    ...
    def __getitem__(self, i):
        ...
        image_slices.append(image)
        mask_mini_slices.append(mask_mini)
        patient_id, slice_id = id_[:4], int(id_[4:6])
        for i in range(self.assist_slice_number, 0, -1):
            underi = max(slice_id - i*self.inter, 0)
            underi_id = patient_id + str(int(underi)).zfill(2)
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            overi_id = patient_id + str(int(overi)).zfill(2)
           # print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_acdc(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'

class ImageToImage2p5DRM_Dynamic_ICH(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=2, assist_slice=3, img_size=256,
        ...
        patient_slice_path = os.path.join(dataset_path, "patient_slice.json")
        self.patient_slice_number = read_slice_number(patient_slice_path)
        self.assist_slice_number = 4 # 3for {1, 3}, 6 for {1,3,5} , 9 for {1, 3, 5, 7} # 9
        self.slice_class_number = ncslice
        self.inter = 1
        self.classes = classes
        self.one_hot_mask = one_hot_mask
    ...
    def __getitem__(self, i):
        ...
        image_slices.append(image)
        mask_mini_slices.append(mask_mini)
        patient_id, slice_id = id_[:3], int(id_[4:7])
        for i in range(self.assist_slice_number, 0, -1):
            underi = max(slice_id - i*self.inter, 0)
            underi_id = patient_id + '_' + str(int(underi)).zfill(3)
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            overi_id = patient_id + '_' + str(int(overi)).zfill(3)
           # print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_ich(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'

class ImageToImage2p5DRM_Dynamic_Prostate(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=2, assist_slice=3, img_size=256,
                 inter=3, ncslice=10, one_hot_mask: int = False) -> None:
        self.dataset_path = dataset_path
        self.img_path = os.path.join(dataset_path, 'imgm1')
        self.imgm2_path = os.path.join(dataset_path, 'imgm2')
        self.label_path = os.path.join(dataset_path, 'label')
        patient_slice_path = os.path.join(dataset_path, "patient_slice.json")
        self.patient_slice_number = read_slice_number(patient_slice_path)
        self.assist_slice_number = 9 # 3for {1, 3}, 6 for {1,3,5} , 9 for {1, 3, 5, 7}
        self.slice_class_number = ncslice
        self.inter = 1
        self.classes = classes
        self.one_hot_mask = one_hot_mask
    ...
    def __getitem__(self, i):
        id_ = self.ids[i]
        image = cv2.imread(os.path.join(self.img_path, id_ + '.png'), 0)
        imagem2 = cv2.imread(os.path.join(self.imgm2_path, id_ + '.png'), 0)
        mask = cv2.imread(os.path.join(self.label_path, id_ + '.png'), 0)
        # correct dimensions if needed
        image, mask = correct_dims(image, mask)
        imagem2, _ = correct_dims(imagem2, mask)

        if self.classes == 2:
            mask[mask > 1] = 1
        if self.joint_transform:
            image, mask, mask_mini, random_dict = self.joint_transform(image, mask)
            imagem2 = self.joint_transform.transform_image(imagem2, random_dict)
            image = torch.cat((image, imagem2), 0)
        if self.one_hot_mask:
            assert self.one_hot_mask > 0, 'one_hot_mask must be nonnegative'
            mask = torch.zeros((self.one_hot_mask, mask.shape[1], mask.shape[2])).scatter_(0, mask.long(), 1)
            mask_mini = torch.zeros((self.one_hot_mask, mask_mini.shape[1], mask_mini.shape[2])).scatter_(0, mask_mini.long(), 1)
        ...
        for i in range(self.assist_slice_number, 0, -1):
            ...
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            imagem2i = cv2.imread(os.path.join(self.imgm2_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
            imagem2i, _ = correct_dims(imagem2i, maski)
            if self.classes == 2:
                maski[maski > 1] = 1
            if self.joint_transform:
                imagei, mask_minii = self.joint_transform.transform_image_mask(imagei, maski, random_dict)
                imagem2i  = self.joint_transform.transform_image(imagem2i, random_dict)
                imagei = torch.cat((imagei, imagem2i), 0)
            image_slices.append(imagei)
            mask_mini_slices.append(mask_minii)
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            ...
            #print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            imagem2i = cv2.imread(os.path.join(self.imgm2_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
            imagem2i, _ = correct_dims(imagem2i, maski)
            if self.classes == 2:
            ...
            image_slices.append(imagei)
            mask_mini_slices.append(mask_minii)
        image_slices = torch.stack(image_slices, dim=0) # k 2 h w
        mask_mini_slices = torch.stack(mask_mini_slices, dim=0)
        # ------------------------ find the object bbox ----------------------------
        bbox = list()
        label = list()
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_prostate(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'

class ImageToImage2p5DRM_Dynamic_INSTANCE(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __init__(self, dataset_path: str, split='train1', joint_transform: Callable = None, classes=2, assist_slice=3, img_size=256,
        ...
        patient_slice_path = os.path.join(dataset_path, "patient_slice.json")
        self.patient_slice_number = read_slice_number(patient_slice_path)
        self.assist_slice_number = 9 # 3for {1, 3}, 6 for {1,3,5} , 9 for {1, 3, 5, 7} #9
        self.slice_class_number = ncslice
        self.inter = 1
        self.classes = classes
        self.one_hot_mask = one_hot_mask
...
class ImageToImage2p5DRM_Dynamic_CHAOS(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __getitem__(self, i):
        ...
        image_slices.append(image)
        mask_mini_slices.append(mask_mini)
        patient_id, slice_id = id_.split('_')[0], int(id_.split('_')[-1])
        for i in range(self.assist_slice_number, 0, -1):
            underi = max(slice_id - i*self.inter, 0)
            underi_id = patient_id + '_' + str(int(underi)).zfill(4)
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            overi_id = patient_id + '_' + str(int(overi)).zfill(4)
            #print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_chaos(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'


class ImageToImage2p5DRM_Dynamic_Task09Spleen(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __getitem__(self, i):
        ...
        for i in range(self.assist_slice_number, 0, -1):
            underi = max(slice_id - i*self.inter, 0)
            underi_id = patient_id + '_' + str(int(underi))
            #print("underid", underi_id)
            imagei = cv2.imread(os.path.join(self.img_path, underi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, underi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[patient_id]-1)
            overi_id = patient_id + '_' + str(int(overi))
            #print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
            imagei, maski = correct_dims(imagei, maski)
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_task09spleen(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'


class ImageToImage2p5DRM_Dynamic_MosMed(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __getitem__(self, i):
        ...
        for i in range(self.assist_slice_number, 0, -1):
            ...
            mask_mini_slices.append(mask_minii)
        for i in range(1, self.assist_slice_number+1):
            overi = min(slice_id + i*self.inter, self.patient_slice_number[str(int(patient_id))]-1)
            overi_id = patient_id + '_' + str(int(overi))
            #print("overid", overi_id)
            imagei = cv2.imread(os.path.join(self.img_path, overi_id + '.png'), 0)
            maski = cv2.imread(os.path.join(self.label_path, overi_id + '.png'), 0)
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_MosMed(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'


class ImageToImage2p5DRM_Dynamic_LITS(Dataset):
    """
    dynamic select the number of assist slices and the spacing between slices
    Reads the images and applies the augmentation transform on them.
    Usage:
    ...
    def __getitem__(self, i):
        ...
        if len(bbox) > 0:
            ...
            bbox = np.array([])
            label = np.array([])
        gt_class_loc, gt_class_label = obtain_gt_loc_lits(bbox, label, self.classes)
        # ----------------------------------------------------------------------------
        return image, mask, mask_mini_slices, image_slices, gt_class_loc, gt_class_label, id_ + '.png'
